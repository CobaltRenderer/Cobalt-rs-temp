...
pub struct UnallocatedTextureBuffer2DArray<'a> {
    texture_buffer: TextureBuffer2DArray,
    _initial_data: std::marker::PhantomData<&'a i32>,
}

impl<'a> UnallocatedTextureBuffer2DArray<'a> {
    pub(crate) fn new(
        handle: sys::Cobalt_TextureBuffer2DArray,
        renderer_internal: Arc<RendererInternal>,
    ) -> Self {
        UnallocatedTextureBuffer2DArray {
            texture_buffer: TextureBuffer2DArray::new(handle, renderer_internal),
    ...
    pub fn allocate_memory(self) -> RendererResult<TextureBuffer2DArray> {
        unsafe {
            return_on_failure!(sys::Cobalt_TextureBuffer2DArray_AllocateMemory(
                self.texture_buffer.handle
            ))
        }
