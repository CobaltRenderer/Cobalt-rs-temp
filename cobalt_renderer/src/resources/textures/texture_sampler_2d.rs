...
/// Sampler for [`TextureBuffer2D`](`super::TextureBuffer2D`)
pub struct TextureSampler2D {
    pub(crate) handle: sys::Cobalt_TextureSampler2D,
    _renderer: Arc<RendererInternal>,
}

impl TextureSampler2D {
    pub(crate) fn new(
        handle: sys::Cobalt_TextureSampler2D,
    ...
    pub fn set_texture_wrap_mode(
        ...
        unsafe {
            sys::Cobalt_TextureSampler2D_SetTextureWrapMode(
                self.handle,
    ...
    pub fn set_texture_filter_mode(
        ...
        unsafe {
            sys::Cobalt_TextureSampler2D_SetTextureFilterMode(
                self.handle,
    ...
    pub fn set_texture_mipmap_mode(&mut self, mipmap_mode: MipmapMode) {
        unsafe {
            sys::Cobalt_TextureSampler2D_SetTextureMipmapMode(
                self.handle,
    ...
    pub fn set_texture_level_mapping(&mut self, min_level: f32, max_level: f32, level_bias: f32) {
        unsafe {
            sys::Cobalt_TextureSampler2D_SetMipmapLevelMapping(
                self.handle,
                min_level,
                max_level,
                level_bias,
    ...
    pub fn set_anisotropic_filter_mode(&mut self, enabled: bool, max_anisotropy: Option<i32>) {
        unsafe {
            sys::Cobalt_TextureSampler2D_SetAnisotropicFilterMode(
                self.handle,
                if enabled { 1 } else { 0 },
...
impl Drop for TextureSampler2D {
    fn drop(&mut self) {
        unsafe {
            sys::Cobalt_TextureSampler2D_Delete(self.handle);
        }
    }
}

unsafe impl Send for TextureSampler2D {}
unsafe impl Sync for TextureSampler2D {}
