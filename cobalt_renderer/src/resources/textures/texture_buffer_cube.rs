...
impl TextureBufferCube {
    ...
    pub fn allocated_image_format(&self) -> ImageFormat {
        let value = unsafe { sys::Cobalt_TextureBufferCube_AllocatedImageFormat(self.handle) };
        ImageFormat::try_from_primitive(value as i32).unwrap()
    }

    pub fn allocated_data_format(&self) -> DataFormat {
        let value = unsafe { sys::Cobalt_TextureBufferCube_AllocatedDataFormat(self.handle) };
        DataFormat::try_from_primitive(value as i32).unwrap()
    }

    pub fn mipmap_level_count(&self) -> i32 {
        unsafe { sys::Cobalt_TextureBufferCube_MipmapLevelCount(self.handle) }
    }

    pub fn mipmap_level_dimensions(&self, mipmap_level: i32) -> [u32; 2] {
        unsafe {
            let mut dimensions: [u32; 2] = [0; 2];
            sys::Cobalt_TextureBufferCube_MipmapLevelDimensions(
                self.handle,
                mipmap_level,
                &mut dimensions,
            );
    ...
    pub fn queue_data_update<S: Sized>(
        ...
        unsafe {
            ...
            return_on_failure!(sys::Cobalt_TextureBufferCube_QueueDataUpdate(
                self.handle,
                source_buffer.as_ptr() as *const std::ffi::c_void,
                core::mem::size_of_val(source_buffer),
                image_format as sys::Cobalt_SourceImageFormat,
...
impl TextureBuffer for TextureBufferCube {
    ...
    fn bind_to_state_container(&self, texture_id: TextureId, container: &mut impl StateContainer) {
        unsafe {
            sys::Cobalt_StateContainer_BindTextureCube(
...
impl Drop for TextureBufferCube {
    fn drop(&mut self) {
        unsafe {
            sys::Cobalt_TextureBufferCube_Delete(self.handle);
        }
    }
}

unsafe impl Send for TextureBufferCube {}
unsafe impl Sync for TextureBufferCube {}
